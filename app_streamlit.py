...
with st.sidebar:
    ...
    # Cloud safety caps (you can increase for self-hosted)
    max_items_per_cell = st.slider("Cap: S1 items per cell", 10, 60, 30, 5)
    max_cells = st.slider("Cap: max cells per run", 100, 1500, 800, 50)

    run_btn = st.button("🔍 Run Scan")

st.markdown(
"""
This tool searches recent Sentinel-1 RTC scenes via STAC (Microsoft Planetary Computer),
derives a current water mask (VV/VH), compares it to **JRC Global Surface Water occurrence**,
and flags cells with statistically significant **new water**.
"""
)
...
if run_btn:
    ...
    with st.spinner("Scanning… this may take a few minutes depending on region and grid size"):
    ...
    if df.empty:
        ...
        view_state = pdk.ViewState(latitude=(lat_min+lat_max)/2, longitude=(lon_min+lon_max)/2, zoom=3)
        ...
        st.download_button("Download GeoJSON", data=json.dumps(gj).encode("utf-8"),
